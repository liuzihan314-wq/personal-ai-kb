...
class RawPaths:
    """Paths for the three files belonging to one Raw document record."""
...
class RawStorage:
    """Store one PDF, its extracted text, and readable metadata per document.
    ...
    def paths_for(self, document_id: str) -> RawPaths:
    ...
    def store(self, document: UnifiedDocument, original_bytes: bytes) -> UnifiedDocument:
        ...
        paths = self.paths_for(document.id)
